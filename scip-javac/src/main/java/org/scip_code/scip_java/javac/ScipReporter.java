 ...
 */
public class ScipReporter {
  private final Trees trees;

  public ScipReporter(Trees trees) {
    this.trees = trees;
  }
  ...
  public void exception(Throwable e, Tree tree, CompilationUnitTree root) {
    ...
    writer.println(
        "Please report a bug to https://github.com/scip-code/scip-java with the stack trace"
            + " above.");
  ...
  public void error(String message, Tree tree, CompilationUnitTree root) {
    ...
    trees.printMessage(Diagnostic.Kind.ERROR, String.format("scip-javac: %s", message), tree, root);
  }
}
