...
class ScipGradlePlugin extends Plugin[Project] {
  import Logging._
  ...
  override def apply(project: Project): Unit = {
    project.afterEvaluate { project =>
      ...
      val targetRoot = extra
        .getProperties()
        .asScala
        .getOrElse("scipTarget", project.getBuildDir())
      ...
      // This list is built progressively as we check for java and kotlin
      // plugins
        ...
        val compilerPluginAdded =
          try {
            ...
            case exc: Exception =>
              ...
              // SCIP output) and the post-build check in
              // `GradleBuildTool` will surface a clearer error.
              warn(
                s"scip-java: failed to attach SCIP compiler plugin to project '${project
        ...
        project
          ...
          .configureEach { task =>
            ...
            if (compilerPluginAdded) {
              ...
              if (!args.asScala.exists(_.startsWith("-Xplugin:scip"))) {
                args.addAll(
                  List(
                    ...
                    s"-Xplugin:scip -targetroot:$targetRoot -sourceroot:$sourceRoot -randomtimestamp=${System
                        .nanoTime()}"
                  ).asJava
                )
              }
...
private object Logging {
  def info(msg: Any*) = System
    .err
    .println(s"[INFO] [scip-java.gradle] ${msg.mkString(" ")}")

  def warn(msg: Any*) = System
    .err
    .println(s"[WARNING] [scip-java.gradle] ${msg.mkString(" ")}")
