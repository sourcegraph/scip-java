...
/**
 * `scip-java aggregate`: aggregates per-source SCIP shards into a single
 * SCIP index file.
 */
class AggregateCommand :
    CliktCommand(name = "aggregate") {
    ...
    private val parallel: Boolean by option(
        "--parallel",
        "--no-parallel",
        help = "Whether to process the SCIP shards in parallel.",
    ).flag("--no-parallel", default = true)

    private val emitInverseRelationships: Boolean by option(
    ...
    private val targetrootOption: List<Path> by option(
        "--targetroot",
        help = "Directories that contain SCIP shards.",
    ).path().multiple()

    private val targetrootArgs: List<Path> by argument().path().multiple()

    private val allowEmptyIndex: Boolean by option(
        "--allow-empty-index",
        help =
    ...
    private val allowExportingGlobalSymbolsFromDirectoryEntries: Boolean by option(
