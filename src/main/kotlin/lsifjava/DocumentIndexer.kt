...
class DocumentIndexer(
    ...
    private var task: JavacTask? = null
    ...
    internal fun emitUse(use: Range, def: Range, defPath: Path) {
        referencesBacklog.add {
            ...
            val link = path.toString().replaceFirst("^" + projectRoot.toRegex(), ".") + ":" + humanRange(use) + " -> " + defPath.toString().replaceFirst("^" + projectRoot.toRegex(), ".") + ":" + humanRange(def)
            ...
            rangeIds.add(rangeId)
            val set = meta.referenceRangeIds.getOrDefault(documentId, HashSet())
            set.add(rangeId)
            meta.referenceRangeIds[documentId] = set
        }
    }
    
    private fun nameRange(a: Range, name: String): Range {
        return Range(
            Position(
                a.start.line,
                a.start.character
