...
import com.sourcegraph.internal.semanticdb_javac.Semanticdb.SymbolInformation.Kind;
import com.sourcegraph.internal.semanticdb_javac.Semanticdb.SymbolInformation.Property;
import com.sourcegraph.internal.semanticdb_javac.Semanticdb.SymbolOccurrence.Role;
...
import static com.sourcegraph.internal.semanticdb_javac.SemanticdbBuilders.*;
import static com.sourcegraph.internal.semanticdb_javac.SemanticdbTypeVisitor.ARRAY_SYMBOL;
...
public class SemanticdbVisitor extends TreePathScanner<Void, Void> {
    ...
    if (node instanceof JCTree.JCClassDecl) {
      ...
      for (Symbol.TypeVariableSymbol typeSym : cls.sym.getTypeParameters()) {
        if (i >= typeParameters.size()) {
          ...
          // Failed to reproduce with a minimal source file so we don't have a test case
          // that hits
          // this branch.
          break;
        }
  ...
  public Void visitNewClass(NewClassTree node, Void unused) {
    ...
    // to avoid emitting a reference to the class itself, we manually scan
    // everything
