...
public class ScipTextDocument {
  ...
  // Map from symbols that have a definition occurrence to the list of symbols
  // that have
  // `is_definition` relationships to that symbol.
  // This map is used to add `is_reference` relationships between all symbols in
  // the list so that
  // doing "Find references"
  // on any of those symbols returns occurrences for all of the symbols in the
  // "clique" (per
  ...
  public Semanticdb.TextDocument manifestOccurrencesForSyntheticSymbols(
    ...
    if (semanticdb.getLanguage() != Semanticdb.Language.SCALA) {
      // It's only semanticdb-scalac that emits SymbolInformation for symbols that
      // have no
    ...
    for (Semanticdb.SymbolInformation info : semanticdb.getSymbolsList()) {
      ...
      if (definition != null) {
        // This symbol has a definition so it doesn't need an is_definition
        // relationship.
