...
import com.sourcegraph.internal.semanticdb_javac.Semanticdb.SymbolInformation.Property;
import com.sourcegraph.internal.semanticdb_javac.Semanticdb.*;

import com.sourcegraph.internal.semanticdb_javac.SemanticdbSymbols;

import java.util.*;
import java.util.concurrent.atomic.AtomicInteger;
import java.util.stream.Collectors;

import static com.sourcegraph.internal.semanticdb_javac.SemanticdbBuilders.typeRef;
...
public class SignatureFormatter {
  ...
  private void formatClassSignature(ClassSignature classSignature) {
    ...
    // Determine which parents from ClassSignature.parents are classes or interfaces
    // so we know to
    // use
    // 'extends' or 'implements'.
    // The logic is as follows:
    // 1. If the symbol has type CLASS, there will always be at least 1 parent. For
    // enums, this is
    // java/lang/Enum#, otherwise it is java/lang/Object# if no superclass is
    // specified.
    // Therefore, if the parents list contains java/lang/Object# type or the symbol
    // is an enum,
    // then no superclass was defined and all parents are interfaces and we must
    // print
    // 'implements'
    // followed by all superinterfaces.
    // Else if it is not an enum and the list of non-synthetic parents is non empty,
    // a superclass
    // was specified and we must print it with the 'extends' keyword prefixed,
    // followed by
    // 'implements' and all superinterfaces, if any.
    // 2. If the symbol has type INTERFACE, then any defined parents must also be
    // interfaces, so if
  ...
  private String formatType(Type type) {
    ...
    if (type.hasTypeRef()) {
      ...
      if (typeRef.getSymbol().equals(ARRAY_SYMBOL)) {
        ...
        if (n == 0) {
          // Special-case for Function1[A, B]: don't wrap `A` in parenthesis like this
          // `(A) => B`
