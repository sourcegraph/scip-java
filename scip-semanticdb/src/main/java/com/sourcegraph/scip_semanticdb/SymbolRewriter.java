 ...
 * _root_/com/example/Foo#}) into fully-qualified SCIP symbols (e.g. {@code semanticdb maven
...
final class SymbolRewriter {
  ...
  String rewrite(String symbol) {
    ...
    if (SemanticdbSymbols.isLocal(symbol)) return symbol;
    Package pkg = packages.packageForSymbol(symbol).orElse(Package.EMPTY);
    return "semanticdb " + pkg.scipTypedEncoding() + " " + symbol;
  }
}
