...
import org.jetbrains.kotlin.cli.common.messages.{CompilerMessageSeverity, CompilerMessageSourceLocation, MessageCollector, MessageRenderer}
...
class LsifBuildTool(index: IndexCommand) extends BuildTool("LSIF", index) {
  ...
  private def compileKotlinFiles(deps: Dependencies, config: Config, allKotlinFiles: List[Path]): Try[Unit] = {
    if (allKotlinFiles.isEmpty) return Success()
    val filesPaths = allKotlinFiles.map(_.toString)

    val plugin = Dependencies.resolveDependencies(
      List(s"com.sourcegraph:semanticdb-kotlinc:${BuildInfo.semanticdbKotlincVersion}"), transitive = false)
      .classpath.head

    val self = config.dependencies.head
    val commonKotlinFiles: List[Path] = Dependencies.kotlinMPPCommon(self.groupId, self.artifactId, self.version) match {
      case Some(common) =>
    ...
    val args = ListBuffer[String](
      ...
      s"-Xplugin=$plugin",
      "-P", s"plugin:semanticdb-kotlinc:sourceroot=$sourceroot",
      "-P", s"plugin:semanticdb-kotlinc:targetroot=$targetroot",
      "-classpath",
      deps.classpathSyntax
    )

    if(commonKotlinFiles.nonEmpty) {
      args += s"-Xcommon-sources=${commonKotlinFiles.map(_.toAbsolutePath.toString).mkString(",")}"
    }
    ...
    if (true) println(s"ARGS ${args.mkString(" ")}")

    val exit = new K2JVMCompiler().exec(new MessageCollector {
      ...
      override def report(compilerMessageSeverity: CompilerMessageSeverity, s: String, compilerMessageSourceLocation: CompilerMessageSourceLocation): Unit = {
        if (s.endsWith("without a body must be abstract") || s.endsWith("must have a body")) return // we get these when indexing the stdlib, no other solution found yet
        val msg = MessageRenderer.PLAIN_FULL_PATHS.render(compilerMessageSeverity, s, compilerMessageSourceLocation)
        System.err.println(msg)
        errors.push(msg)
      }
    }, Services.EMPTY, kargs)
    if (exit.getCode == 0)
      Success(())
    else
